...
unsafe extern "C" {
    ...
    fn wayland_display_vk_queue(queue: *mut gstvk::GstVulkanQueue) -> vk::Queue;
...
pub struct RawVk {
    ...
    pub queue: vk::Queue,
    pub gfx_queue: VulkanQueue,
}

/// Per-`waylanddisplaysrc`-element owner of the Vulkan objects the encode path shares.
///
/// **8th gst-wayland-display patch — per-element Vulkan device ownership.** Historically the
...
/// answers `gst.vulkan.{instance,device}` context queries with, and (on teardown, via
/// [`clear`](VulkanShare::clear)) destroys its **own** device. N concurrent Vulkan-encode
...
/// The `repr(C)` overlays (module header) and the `wayland_display_vk_*` C bridge are
/// untouched.
...
impl VulkanShare {
    ...
    /// Destroy this element's owned instance + device (session teardown). Called from the
    /// element's `stop()` alongside `drop(state.display)`, so a retired session takes its
    /// `VkDevice` with it — the gwd half of the per-session failure domain, and what keeps N
    /// sessions from leaking N devices (the old process-global slot leaked by design).
    pub fn clear(&self) {
...
pub fn raw_handles(device: &VulkanDevice) -> Option<RawVk> {
    ...
    unsafe {
        ...
        // Keep the actual GStreamer queue object: VkQueue submission is externally
        // synchronized, so the producer must use the same submit lock as GStreamer.
        let gfx_ptr = gstvk::gst_vulkan_device_select_queue(
            dev_ptr,
            (vk::QueueFlags::GRAPHICS | vk::QueueFlags::COMPUTE).as_raw(),
        );
        if gfx_ptr.is_null() {
            return None;
        }
        ...
        let instance = ash::Instance::load(entry.static_fn(), vk_instance);
        let families = instance.get_physical_device_queue_family_properties(vk_physical);
        let required = vk::QueueFlags::GRAPHICS | vk::QueueFlags::COMPUTE;
        if families
            .get(gfx_queue_family as usize)
            .is_none_or(|family| !family.queue_flags.contains(required))
        {
            drop(from_glib_full::<_, VulkanQueue>(gfx_ptr));
            tracing::error!(
                gfx_queue_family,
                "vulkan_share: selected producer queue is not graphics+compute capable"
            );
            return None;
        }
        let queue = wayland_display_vk_queue(gfx_ptr);
        let gfx_queue: VulkanQueue = from_glib_full(gfx_ptr);
        Some(RawVk {
...
mod tests {
    ...
    /// Per-element ownership, GPU-free half of spec §4 rung 1: two `VulkanShare`s are
    /// independent objects (distinct backing storage), each starts with no device, and
    /// clearing / mutating one never touches the other. The full "two elements mint **distinct**
    /// `GstVulkanDevice` pointers" assertion needs a real GPU and belongs to the Tower soak
    /// (§4 rung 1 / §7 cross-session distinctness log) — it cannot run in a headless unit test
    /// because `ensure_owned_device` opens a real `VkDevice`.
    #[test]
    fn shares_are_independent_per_element() {
        let a = VulkanShare::new();
        let b = VulkanShare::new();
        // Distinct allocations — not the old process-global singleton.
        assert!(!Arc::ptr_eq(&a, &b));
        // Fresh shares own no device until first use.
        assert!(a.shared_device().is_none());
        assert!(b.shared_device().is_none());
        // Clearing one is a no-op on the other (both remain empty; no shared slot).
        a.clear();
        assert!(a.shared_device().is_none());
        assert!(b.shared_device().is_none());
    }
